...
(function(document) {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('[data-sa-graph-url]').forEach(function(graph) {
      ...
      iframe.setAttribute('src', url + append + 'embed=true&graph_id=' + counter)
    ...
    window.addEventListener('message', function(event) {
      if (typeof event.data !== 'object') {
        ...
        document.getElementById('sa-graph-' + event.data.id).height = event.data.height + 'px'
        ...
        if (selector) document.querySelectorAll(selector).forEach(function(pageViewsElement) {
